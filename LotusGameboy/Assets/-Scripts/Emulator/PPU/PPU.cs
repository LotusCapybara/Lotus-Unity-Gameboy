...
namespace Lotus.GameboyEmulator
{
    ...
    public class PPU
    {
        ...
        public static int TOTAL_PIXELS = SCREEN_WIDTH * SCREEN_HEIGHT;
        ...
        private void RenderWindow()
        {
            ...
            {
                byte x = p >= wx ? (byte)(p - wx) : (byte) (p + wx);

                if ((p & 0x7) == 0 || ((p + wx) & 0x7) == 0) 
                {
                    ...
                    for (int p = 0; p < 8; p++) 
                    {
                        if (x + p >= 0 && x + p < SCREEN_WIDTH)
                        {
                            ...
                            if (isPixelVisible && isPixelAboveBG || isWhiteBG) 
                            {
                                ...
                                byte palette = Tools.GetBIT(attr, 4) == 1 ? _bus.objPalette1 : _bus.objPalette0; 
